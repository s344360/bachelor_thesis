...
for ca_index = 1:size(ca_cell)
   %ca_index=1;
    for n = 1:mal
        n;
        %verschieben_zahler=richtich+240+n;
        nummer= richtich+n;
        plaz =sprintf('GP_m:0.55_c:%s_tout:%s_ZD:1200_n:%d.txt', ca_cell{ca_index,1},test17{n,1},nummer-1); 
        plaz
        fid= fopen(plaz);
        temp{nummer,1}= textscan(fid,'%s %s %s %s %s %s %s %s %s');
        fclose(fid);
        
        zaehler{nummer,1} = 0;
        for c = 1:size(temp{nummer,1}{9})
            if ( temp{nummer,1}{9}{c} == "OFPT_FLOW_MOD" )              
                zaehler{nummer,1}= zaehler{nummer,1}+1;
            end
        end
    end 
